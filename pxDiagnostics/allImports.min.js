...
font:e.boldFont,pixelSize:12,alignHorizontal:a.alignHorizontal.CENTER,textColor:e.charcoalGreyColor});m=a.create({t:"rect",parent:l,x:l.w*q,w:1,h:l.h,fillColor:k});k=a.create({t:"textBox",parent:l,x:m.x+5,y:5,w:350,h:25,text:f,font:e.regularFont,pixelSize:11,alignHorizontal:a.alignHorizontal.LEFT,textColor:e.blackSolidColor,wordWrap:!0});f=k.measureText(f).bounds;if(l.h<f.y2-f.y1){var t=l.h;l.h=f.y2-f.y1+10;m.h=l.h;b.y+=(l.h-t)/2;v+=l.h}else v+=25;c++;d();return k};this.addRowLeft=function(b,c,m){var l=
...
w.x=(v.w-p-r.headerGap)/2;w.w=p+r.headerGap;return b};this.selectTab=function(a){f[a].select();m=a};g.on("onPreKeyDown",function(a){switch(a.keyCode){case n.ENTER:null==A&&(A=setTimeout(function(){if(1E3>Date.now()-l)try{d({message:"Rebooting.....",w:300,h:100,x:(g.w-250)/2,y:(g.h-120)/2});var a=global.registry.scene.getService("systemService");if(a){a.setApiVersionNumber(17);var b=a.callMethod("reboot");!1===JSON.parse(b).success&&console.log("Reboot Failed!")}else console.log("unable to initialize system_15 service to get device details")}catch(H){console.log("caught exception in reboot box"),
